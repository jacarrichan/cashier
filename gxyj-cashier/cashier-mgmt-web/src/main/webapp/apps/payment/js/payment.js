...
define(function(require, exports, module) {
	...
	PageScript.prototype.loadData = function(){
		...
		if($('#channelCode').val().trim() != null && $('#channelCode').val().trim()!='' && checkCharacter($('#channelCode').val().trim())) {
			alert("查询条件支付渠道编码不能有特殊字符！");
			return ;
		}
		else if ($('#channelName').val().trim() != null && $('#channelName').val().trim()!='' && checkCharacter($('#channelName').val().trim())) {
			alert("查询条件支付渠道名称不能有特殊字符！");
			return ;
		}
		/*else if ($('#merchantId').val().trim() != null && $('#merchantId').val().trim()!='' && checkCharacter($('#merchantId').val().trim())) {
			alert("查询条件商户ID不能有特殊字符！");
			return ;
		}*/
		
		var zuiLoad = new $.ZuiLoader().show('数据加载中...');
		ajax.post({
			url: 'payment/channelPage',
			data: {"channelCode":$('#channelCode').val().trim(),
				...
				"channelPlatform":$('#channelPlatform').val(),
		...
		if(data.size && data.list){
			...
			for(var i = 0; i < data.size; i++){
				...
				trHtmls += laytpl('list-tr.tpl').render({
					...
					"channelType": page.channelTypeModel[obj.channelType],
					/*"merchantId" : obj.merchantId, */
					"appId": obj.appId,
					"privateKey": obj.privateKey,
					/*"publicKey": obj.publicKey,*/
					"channelPlatform" : page.statePlatform[obj.channelPlatform],
					"usingDate": (function(){
					...
					"buttons": (function(){
						...
						if(obj.usingStatus == '0') { // 未启用的记录可以看到修改，删除按钮
	...
	PageScript.prototype.bindEvent = function(){
		...
		$('.result-panel').on('click', '.btn-list-alt', function(){
			...
			window['$depositDialog'] = new $.zui.ModalTrigger({
				...
				width: 900,
				height: 600,
				iframe: utils.formatByJson('./payment_vind.html?type=list&channelId={rowId}&channelName={channelName}', obj)					
			});
			// 显示弹出框
			$depositDialog.show();
			// 绑定内部按钮事件
		...
		$('.btn-add').on('click', function(){
			...
			window['$depositDialog'] = new $.zui.ModalTrigger({
				...
				width: 800,
				height: 600,
				iframe: './add_edit_payment.html?type=add'
			});
			// 显示弹出框
			$depositDialog.show();
			// 绑定内部按钮事件
		...
		$('.result-panel').on('click', '.btn-edit', function(){
			...
			window['$depositDialog'] = new $.zui.ModalTrigger({
				...
				width: 800,
				height: 600,
				iframe: utils.formatByJson('./add_edit_payment.html?type=edit&rowId={rowId}&channelCode={channelCode}&channelName={channelName}&usingStatus={usingStatus}&appId={appId}&privateKey={privateKey}&publicKey={publicKey}&channelPlatform={channelPlatform}&channelType={channelType}', obj)
			});
			// 显示弹出框
			$depositDialog.show();
			// 绑定内部按钮事件
		...
		$('.result-panel').on('click', '.btn-cogs', function(){
			...
			window['$depositDialog'] = new $.zui.ModalTrigger({
				...
				width: 800,
				height: 600,
				iframe: utils.formatByJson('./payment_vind_add_edit.html?type=add&channelId={rowId}&channelName={channelName}', obj)
			});
			// 显示弹出框
			$depositDialog.show();
			// 绑定内部按钮事件
		...
		$('.result-panel').on('click', '.btn-off', function(){
			...
			var obj = page.RowData["row-" + rowId];
			var confirm_txt = "";
			confirm_txt = (obj.usingStatus == "0") ? "开启" : "关闭";
		    $.confirm({
				title: '提示',
		...
		$('.clear-btn').on('click', function(){ 
			...
			$("#btnradio").click();
			/*$("#merchantId").val("");*/
			$("#channelPlatform").find("option[value='']").attr("selected",true);

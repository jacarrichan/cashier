define(function(require, exports, module) {
	...
	PageScript.prototype.loadData = function(){
		...
		ajax.post({
			url: 'order/refundQueryList',
			data: {
				"channelId":$('#channelId').val(), 
				"refundId" : $('#refundId').val(),
				"orgnOrderId" : $('#orgnOrderId').val(),
				"mallId" : $('#mallId').val(),
		...
		if(data.size && data.list){
			...
			for(var i = 0; i < data.size; i++){
				...
				trHtmls += laytpl('list-tr.tpl').render({
					...
					"orgnOrderId": obj.orgnOrderId,
					"refundDate": utils.formatDate(obj.refundDate,"yyyy-MM-dd hh:mm:ss"),
					"refundAmt": obj.refundAmt.toFixed(2),
					...
					"buttons": (function(){
						var btnHtml = '';
							...
							if(obj.procState !="00" && obj.procState !="01" && obj.procState !="02"){
						 	 btnHtml += laytpl('list-btn.tpl').render({
	...
	PageScript.prototype.bindEvent = function(){
		...
		$('.result-panel').on('click', '.btn-manualQuery', function(){
		    ...
		    $.confirm({
				title: '提示',
				yesText: '发起查询',
				msg: '是否向支付渠道发起该退款查询<br>确认要发起查询吗？',
				yesClick: function($modal){
					if(rowId){
						...
						if(obj){
							...
							ajax.post({
								url: 'order/qryRefundPayResult',
								data: {
									"transId": obj.transId ,
									"refundId": obj.refundId,
									"payerInstiNo": obj.payerInstiNo,
									"channelCd": obj.remark,
								}
							}).done(function(res, rtn, state, msg){
//									if(state){
//										layer_msg.msg("手工查询成功",2000);
//										page.loadData();
//									} else {
...
//									}
		...
		$('.result-panel').on('click', '.btn-detail', function(){
			...
			window['$depositDialog'] = new $.zui.ModalTrigger({
				...
				width: 800,
				height: 600,
				iframe: utils.formatByJson('./detail_refundOrder.html?type=edit&transId={transId}&mallId={mallId}&channelCd={channelCd}&refundId={refundId}&orgnOrderId={orgnOrderId}&refundDate={refundDate}&refundAmt={refundAmt}&payerInstiNm={payerInstiNm}&procState={procState}&remark={remark}', obj)
			});
			// 显示弹出框
			$depositDialog.show();
			// 绑定内部按钮事件

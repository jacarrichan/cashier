	...
	PageScript.prototype.bindEvent = function(){
		...
		$('.btn-add').on('click', function(){
			...
			window['$depositDialog'] = new $.zui.ModalTrigger({
				...
				width: 800,
				height: 600,
				iframe: './roleinfo_addedit.html?type=add'
			});
			// 显示弹出框
			$depositDialog.show();
			// 绑定内部按钮事件
		...
		$('.result-panel').on('click', '.btn-edit', function(){
			...
			window['$depositDialog'] = new $.zui.ModalTrigger({
				...
				width: 800,
				height: 600,
				iframe: utils.formatByJson('./roleinfo_addedit.html?type=edit&rowId={rowId}&roleId={roleId}&roleName={roleName}&brchId={brchId}&brchName={brchName}', obj)
			});
			// 显示弹出框
			$depositDialog.show();
			// 绑定内部按钮事件

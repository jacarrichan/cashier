...
define(function(require, exports, module){
	...
	PageScript.prototype.initBusiChannelSelect = function(ops){
		// 检查参数
		ops = ops || {};
		if(!ops.selector){
			error("选择列表组件初始化失败：参数错误");
			log.error("选择列表组件失败，参数不符合要求，请检查：", ops);
			return;
		}
		
		_loadBusiChannelList(ops);
	};
	...
	PageScript.prototype.initPayerInstiNoSelect = function(ops){
		// 检查参数
		ops = ops || {};
		if(!ops.selector){
			error("支付渠道选择列表组件初始化失败：参数错误");
			log.error("初始化支付渠道选择列表组件失败，参数不符合要求，请检查：", ops);
			return;
		}
		_loadPayerInstiNoList(ops);
	};
	
	
	PageScript.prototype.initPaymentSelect = function(ops){
		// 检查参数
		ops = ops || {};
		if(!ops.selector){
			error("选择列表组件初始化失败：参数错误");
			log.error("初始选择列表组件失败，参数不符合要求，请检查：", ops);
			return;
		}
		
		_loadPaymentList(ops);
	};
	...
	function _loadMallList(ops){
		...
		ajax.post({
			url: 'relt/mallInfo',
			data: { mallId: ops.selectValue}
		}).done(function(res, rtn, state, message){
			if(state){
				$mall.empty();
				if(ops.hasAll){
					$mall.append(utils.parsetpl('<option value="" data-text="{name}" {attr}>{name}</option>', {
						attr: ops.selectValue ? '' : 'selected="selected"',
								name:ops.allName ? ops.allName: '--请选择--' 
					}));
				}
				var attr = '', obj;
				for(var i = 0; i < rtn.data.length; i++){
	...
	function _loadBusiChannelList(ops){
		var $mall = $(ops.selector);
		...
		ajax.post({
			...
			if(state){
				...
				for(var i = 0; i < rtn.data.length; i++){
					obj = rtn.data[i];
					$mall.append(utils.parsetpl('<option value="{code}" data-text="{name}" {attr}>{name}</option>', {
						code: obj.channelCode,
						name: obj.channelName,
						attr: ops.selectValue == obj.channelCode ? 'selected="selected"' : ''
					}));
				}
				$mall.trigger('chosen:updated');
	...
	function _loadPayerInstiNoList(ops){
		var $payerInstiNo = $(ops.selector);
		ajax.post({
			...
			if(state){
				$payerInstiNo.empty();
				if(ops.hasAll){
					$payerInstiNo.append(utils.parsetpl('<option value="" data-text="全部" {attr}>全部</option>', {
						attr: ops.selectValue ? '' : 'selected="selected"'
					}));
				}
				var attr = '', obj;
				for(var i = 0; i < rtn.data.length; i++){
					obj = rtn.data[i];
					$payerInstiNo.append(utils.parsetpl('<option value="{code}" data-text="{name}" {attr}>{name}</option>', {
						code: obj.channelCode,
						name: obj.channelName,
						attr: ops.selectValue == obj.channelCode ? 'selected="selected"' : ''
					}));
				}
				$payerInstiNo.trigger('chosen:updated');
				
				setTimeout(function(){
					ops.success && ops.success($payerInstiNo[0]);
				}, 100);
				
				// Chosen绑定改变事件
				if($payerInstiNo.data('ac') != 'on'){
					$payerInstiNo.data('ac', 'on');
					$payerInstiNo.on('change', function(){
						var bid = $(this).val();
						ops.change && ops.change($payerInstiNo[0], bid);
					});
				}
			}
		}).fail(function(e){
	...
	function _loadPaymentList(ops){
		...
		ajax.post({
			...
			if(state){
				$store.empty();
				if(ops.hasAll){
					$store.append(utils.parsetpl('<option value="" data-text="{name}" {attr}>{name}</option>', {
						attr: ops.selectValue ? '' : 'selected="selected"',
						name:ops.allName ? ops.allName: '--请选择--' 
					}));
				}

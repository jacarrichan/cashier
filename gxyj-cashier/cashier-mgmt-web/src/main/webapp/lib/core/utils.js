...
var json = require("./json"),
    ...
    dateToString : function(data){
		...
		var	fullDate = data.substring(0, 10).replace(/-/g, "/");
	    var	fullTime = data.substring(11, 19);
	    var	date = new Date(fullDate+" "+fullTime);
...
//	    		}    			
    ...
    /**
     * 保证金模块修改页面专用金额格式化方法（css样式有调整不向右对齐）
     */
    parseM: function(num, currency, className){
    	num = num || "0.00";
    	if(/^(?:-?\d+|-?\d{1,3}(?:,\d{3})+)?(?:\.\d+)?$/.test(num)){
    		var str = "" + num;
    		if(str.indexOf('.') == -1){
    		...
    		str = splitx(before) + '.' + splity(after);
    		num = (currency || "") + '<span class="money-html '+(className || "color-primary")+' ">' + str + '</span>';
    	}
    	return num;
    },

...
define(function(require, exports, module) {
	...
	function ZuiValidator(selector){
		...
		that.settings = {
			...
			message: {
				...
				spec:"不能含有特殊字符",
				bothSideBlank:"输入值两边不能有空格",
				charAndNumber:"只能输入字母和数字",
				chinaCharNumber: "只能输入中文,字母和数字",
			}
		}
	...
	ZuiValidator.prototype = {
		...
		validate: function( arg, arg1 ){
			var that = this;
			if(arg1){
				that.showDirect = arg1;
			}
		...
		showError: function(ele){
			...
			ele.dom.parent.children("input,select,textarea").tooltip('destroy')
				.data('toggle', 'tooltip')
				.data('placement', that.showDirect? that.showDirect: "right" )
				.attr('title', msg || that.defaultMsg)
				.tooltip('show');
			if(that.showDirect == "right" && $("#"+ele.dom.eleName).get(0).tagName == "SELECT"){
				var left = $("#"+ele.dom.eleName).width() + $("#"+ele.dom.eleName).siblings(".chosen-container").offset().left + 18 +"px";
				$("#"+ele.dom.eleName).siblings(".tooltip").css({
					"left":left,
					"top":"5px"
				})
			}
		...
		hideError: function(dom){
			...
			dom.parent.children("input,select,textarea").tooltip('destroy');
		},
		...
		methods: {
			...
			isExist: function( value, element, param ) {
				...
				if(param.url && param.data){
					...
					ajax.syncAjax({
						url: param.url,
						data: param.data,
						tokenUrl: param.tokenUrl? param.tokenUrl:false,
			...
			spec: function ( value, element) {
				var myReg = /^[^@\/\'\\\"#$%&\^\*]+$/;
				 
		        return this.optional( element ) || myReg.test(value);
			},
			
			bothSideBlank: function(value, element) { /* 输入字符串两边不能有空格  */ 
				var myReg = /^[\s　]|[ ]$/gi;
				return this.optional( element ) || !myReg.test(value);
			},
			
			charAndNumber: function(value, element) { /* 只包含字母和数字验证*/
				var myReg = /^[A-Za-z0-9]+$/;
				return this.optional( element ) || myReg.test(value);
			},
			
			chinaCharNumber : function(value, element) { /* 只包含中文，字母和数字验证*/

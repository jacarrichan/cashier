...
define(function(require, exports, module) {
	...
	var ultpl = 
	    ...
	    '<li><input class="page pageinput" value="{curr_page}" {input_disabled}/></li>' +
	    '<li>页，每页</li>' +
	    '<li><select class="pager-chosen"><option value="10">10</option><option value="50">50</option><option value="100">100</option></select></li>' +
	    '<li>条，共{totalPages}页，共{totalElements}条记录，完成于：{finishTime}.</li>' +
	...
	ZuiPager.prototype.create = function(selector, page, ops){
		...
		this.$ele = $(selector);
		this.setOption(ops);
		if(this.$ele.length && page && (page.numberOfElements != undefined || page.size != undefined)){
			this.page = page;
			render();
		} else {
		...
		if(!pager.loading){
			try{
				...
				if(this.options.timeout){
					this.data.timeout = this.options.timeout;
				}
				ajax.post(this.data).done(function(res, rtn, state, msg){
					if(state){
						var file = {fileName: rtn.data};
						file = encodeURI(encodeURI(utils.toJSON(file)));
						var url = consts.WEB_ROOT + "m800/f80602";
						...
						if(agent && agent.indexOf('MSIE 9.0') != -1){ // IE9特殊处理
							$iframe.html($form);
						}else if(agent && agent.indexOf('MSIE') != -1){// IE浏览器做特殊处理(解决ie11以下导出不兼容)
							$iframe.html($form);
						} else {
	...
	function render(){
		...
		if (!pager.page.totalPages) {
			pager.page.totalPages = 0;
			pager.page.number = 0;
			pager.page.totalElements = 0;
		}
		
		var totalPages = pager.page.pages? pager.page.pages: pager.page.totalPages,
			currIndex = pager.page.pageNum? pager.page.pageNum -1: pager.page.number,
			totalElements = pager.page.total? pager.page.total : pager.page.totalElements,
			isFirst = pager.page.isFirstPage? pager.page.isFirstPage : pager.page.first,
			isLast = pager.page.isLastPage?pager.page.isLastPage:pager.page.last,
			htmls = '';
		if (!pager.page.totalPages) {
			isFirst = false;
			isLast = false;
		...
		htmls += parsetpl(ultpl, {
			...
			"ctl_show": pager.options.ctlColumn ? '' : 'hidden'
		});
	...
	function renderLiList(currIndex, totalPages){
		...
		if(totalPages <= pager.maxinumLi){
			...
			var currPage = pager.page.pageNum? pager.page.pageNum-1 : pager.page.number;
	...
	function next(){
		var totals = pager.page.total?pager.page.total :pager.page.totalPages;
		if(pager._pageIndex < totals - 1){
			pager._pageIndex++;
			pager.load();
		}
	}

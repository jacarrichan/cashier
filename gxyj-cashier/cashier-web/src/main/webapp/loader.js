...
!(function(global, undefined) {
	...
	if(agent){
		...
		if(ies && ies.indexOf("MSIE") != -1){
			...
			if(Number(v) < 8){
				document.write("<center><h1>浏览器版本过低，请更换浏览器</h1><p>建议使用：谷歌浏览器、火狐浏览器、IE8及以上版本浏览器、360浏览器、QQ浏览器、搜狗浏览器等等</p></center>");
				return;
			}
		}
	}
                ...
                $(function() {
                    ...
                    $(document).bind("keydown", function(e) {
                    	...
                    	if("8" == key && e.target.tagName.toLowerCase() === "body") {
                            e.preventDefault();
                        } else if("116" == key && hurl != seajs.data.base && !hurl.endsWith("/fss/index.html")){
                        	e.preventDefault();
                        }
                    	...
                    	if(fkey !== e.key){
                    		...
                    		if("8" != kcode && top.hasShortKey && top.hasShortKey(ackey, hash)){
                            	top.callShortKey(e, ackey, hash);
                            	e.preventDefault();
                            }
                    		firstKey = {
                            	code: kcode,
